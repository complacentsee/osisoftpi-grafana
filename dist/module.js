...
var PIWebAPIQueryEditor = /*#__PURE__*/function (_PureComponent) {
  ...
  _createClass(PIWebAPIQueryEditor, [{
    ...
    value: function render() {
      ...
      return /*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_1___default().createElement((react__WEBPACK_IMPORTED_MODULE_1___default().Fragment), null, /*#__PURE__*/react__WEBPACK_IMPORTED_MODULE_1___default().createElement(_grafana_ui__WEBPACK_IMPORTED_MODULE_2__.InlineField, {
        label: "Is Pi Point?",
...
var PiWebAPIDatasource = /*#__PURE__*/function (_DataSourceApi) {
  ...
  _createClass(PiWebAPIDatasource, [{
    ...
    value: function query(options) {
      ...
      var observables = ds.getStream(query).map(function (targetResponse) {
        return new rxjs__WEBPACK_IMPORTED_MODULE_3__.Observable(function (subscriber) {
          targetResponse.then(function (targetResponse) {
            var frame = new _grafana_data__WEBPACK_IMPORTED_MODULE_1__.CircularDataFrame({
              append: 'tail',
              capacity: maxPointCapacity
            });
            (0,lodash__WEBPACK_IMPORTED_MODULE_0__.each)(targetResponse, function (item) {
              frame.refId = item.refId;
              frame.name = item.target; //FIXME: add support for other types
              ...
              if (item.WebId && _this3.piwebapiurl) {
                var base_url = window.location.origin;
                var address = base_url.replace(/(^\w+:|^)\/\//, ''); //Note: This only works as a browser side, not as a server side

                address = 'ws://' + address + _this3.url + '/streams/' + item.WebId + '/channel?includeInitialValues=true';
                console.log(address);
                var connection = new WebSocket(address);

                connection.onerror = function (error) {
                  console.log("WebSocket error: ".concat(JSON.stringify(error)));
                };

                connection.onmessage = function (event) {
                  var eventobject = JSON.parse(event.data);
                  var timestamp = eventobject.Items[0].Items[0].Timestamp;
                  var date = new Date(timestamp);
                  var unixTimestamp = date.getTime();
                  var value = eventobject.Items[0].Items[0].Value;
                  frame.add({
                    time: unixTimestamp,
                    value: value
                  });
              ...
              (0,lodash__WEBPACK_IMPORTED_MODULE_0__.each)(item.datapoints, function (datapoint) {
                frame.add({
                  time: datapoint[1],
                  value: datapoint[0]
                });
      ...
      if (annotationOptions.attribute && annotationOptions.attribute.enable) {
        ...
        return this.restBatch(query).then(function (result) {
          ...
          var annotations = (0,lodash__WEBPACK_IMPORTED_MODULE_0__.map)(data.Items, function (item, index) {
            return (0,lodash__WEBPACK_IMPORTED_MODULE_0__.curry)(_this4.eventFrameToAnnotation)(annotationOptions, false, item, valueData.Items[index].Content.Items);
          });

          if (options.annotation.showEndTime) {
            var ends = (0,lodash__WEBPACK_IMPORTED_MODULE_0__.map)(data.Items, function (item, index) {
              return (0,lodash__WEBPACK_IMPORTED_MODULE_0__.curry)(_this4.eventFrameToAnnotation)(annotationOptions, true, item, valueData.Items[index].Content.Items);
            });
        ...
        return this.restGet('/assetdatabases/' + this.afdatabase.webid + '/eventframes?' + filter.join('&')).then(function (result) {
          var annotations = (0,lodash__WEBPACK_IMPORTED_MODULE_0__.map)(result.data.Items, (0,lodash__WEBPACK_IMPORTED_MODULE_0__.curry)(_this4.eventFrameToAnnotation)(annotationOptions, false));

          if (options.annotation.showEndTime) {
            var ends = (0,lodash__WEBPACK_IMPORTED_MODULE_0__.map)(result.data.Items, (0,lodash__WEBPACK_IMPORTED_MODULE_0__.curry)(_this4.eventFrameToAnnotation)(annotationOptions, true));
            (0,lodash__WEBPACK_IMPORTED_MODULE_0__.each)(ends, function (end) {
              annotations.push(end);
            });
          }
    ...
    value: function parsePiPointValueList(value, target, isSummary) {
      ...
      (0,lodash__WEBPACK_IMPORTED_MODULE_0__.each)(value, function (item) {
        // @ts-ignore
        var _this5$noDataReplace = _this5.noDataReplace(isSummary ? item.Value : item, target.summary.nodata, api.parsePiPointValue(isSummary ? item.Value : item, target, isSummary)),
            grafanaDataPoint = _this5$noDataReplace.grafanaDataPoint,
            previousValue = _this5$noDataReplace.previousValue,
            drop = _this5$noDataReplace.drop;
    ...
    value: function getStream(query) {
      ...
      (0,lodash__WEBPACK_IMPORTED_MODULE_0__.each)(query.targets, function (target) {
        ...
        var displayName = target.display ? _this6.templateSrv.replace(target.display, query.scopedVars) : null;
        ...
        if (target.expression) {
          ...
          if (isSummary) {
            url += '/summary' + timeRange + '&intervals=' + query.maxDataPoints + _this6.getSummaryUrl(target.summary);
      ...
      return promises.then(function (webidresponse) {
        ...
        return ds.restBatch(query).then(function (response) {
          ...
          (0,lodash__WEBPACK_IMPORTED_MODULE_0__.each)(response.data, function (value, key) {
            var webID = webidresponse[parseInt(key, 10) - 1].WebId;
