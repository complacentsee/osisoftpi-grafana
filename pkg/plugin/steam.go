...
// TODO: Improvement: Should the user have the option to limite the streaming rate?
...
func (d *Datasource) subscribeToWebsocketChannel(ctx context.Context, Path string, sender *backend.StreamSender, errchan chan error) {
	...
	go d.sendStreamMessagesToSender(ctx, WebID, sender, Path, errchan, d.streamChannels[WebID])
}
...
func (d *Datasource) sendStreamMessagesToSender(ctx context.Context, WebID string, senders *backend.StreamSender, Path string, errchan chan error, streamChannel chan []byte) {
	for {
		select {
		case <-ctx.Done():
		...
		case message, ok := <-streamChannel:
			...
			framedata, _ := convertItemsToDataFrame(frame.Items[0].Name, *items, Type, DigitalState, false)
			d.sendersByWebIDMutex.Lock()
...
func (d *Datasource) addSender(WebID string, sender *backend.StreamSender) {
	d.sendersByWebIDMutex.Lock()
...
func (d *Datasource) removeSender(WebID string, sender *backend.StreamSender) {
	d.sendersByWebIDMutex.Lock()
